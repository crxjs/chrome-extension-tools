...
test('bundles chunks', async () => {
  ...
  expect(chunks.find(byFileName('src/popup.js'))).toBeDefined()
  expect(chunks.length).toBe(1)

  // Assets
  const assets = output.filter(isAsset)
  expect(assets.find(byFileName('manifest.json'))).toBeDefined()
  expect(assets.find(byFileName('src/popup.html'))).toBeDefined()
  expect(assets.find(byFileName('favicon.svg'))).toBeDefined()
