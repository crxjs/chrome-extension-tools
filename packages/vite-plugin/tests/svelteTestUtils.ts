...
export function normalizeSvelteOutput(source: string): string {
  ...
  return source.replace(
    /\b(add_location\$?)\(([^,]+),\s*([^,]+),\s*(\d+),\s*(\d+),\s*(\d+)\)/g,
    (_match, callee, element, file, line, column) => {
      ...
      return `${callee}(${element}, ${file}, ${line}, ${column}, ${normalizedPosition})`
    },
  )
}
