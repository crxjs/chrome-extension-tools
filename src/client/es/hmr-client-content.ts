...
// periodically reconnect to the background to keep it awake
function setupPort() {
  port?.disconnect()
    ...
    case 'update':
      ...
      payload.updates.forEach((update) => {
        if (update.type === 'js-update') {
          ...
          if (el) {
            const newPath = chrome.runtime.getURL(
              `${searchUrl.slice(1)}${
                searchUrl.includes('?') ? '&' : '?'
              }t=${timestamp}`,
            )
...
/**
 * Buffer multiple hot updates triggered by the same src change so that they are
 * invoked in the same order they were sent. (otherwise the order may be
 * inconsistent because of the http request round trip)
 */
  ...
  await Promise.all(
    Array.from(modulesToUpdate).map(async (dep) => {
      ...
      try {
        ...
        warnFailedFetch(e as Error, dep)
      }
    }),
  )
